...
@router.post("", summary="创建维保要求", dependencies=[Depends(verify_token)])
...
@router.post("/batch", summary="批量创建维保要求", dependencies=[Depends(verify_token)])
...
@router.get("", summary="查询所有维保要求", dependencies=[Depends(verify_token)])
...
async def get_requirement(doc_id: str):
    ...
    if not requirement:
        raise HTTPException(status_code=404, detail="维保要求不存在")
    return requirement
...
async def update_requirement(
    ...
    if count == 0:
        raise HTTPException(status_code=404, detail="维保要求不存在或没有变化")
